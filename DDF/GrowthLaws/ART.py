...
stress_function = dolfinx.fem.Function(tensor_space)
stress_function.interpolate(F_g_expression)

F_grr = ufl.conditional(ufl.gt(t, 0),
                        ((stress_function[0,0]/sigma_seta - 1)/2000) + 1, 
                        1)
                        

F_gff = ufl.conditional(ufl.gt(t, 0),
                        ((stress_function[1,1]/sigma_setp - 1)/2000) + 1,
                        1)
...
stress_expression = dolfinx.fem.Expression(P, tensor_space.element.interpolation_points())
...
for i in range(N):
    ...
    stress_function.interpolate(stress_expression)
    ...
    if i < 2995:
        if i % 100 == 0:
            ...
            print("stress_function = ", ddf.eval_expression(stress_function, mesh))
    else:
        print("Step ", i)
        print("F_g = ", ddf.eval_expression(F_g, mesh))
        print("F_g_tot = ", ddf.eval_expression(F_g_tot_function, mesh))
