...
x_right_x = (lambda x : np.isclose(x[0], x_max), 0, dolfinx.default_scalar_type(0.5))
...
'''Neumann'''
neumann_x_right_x  = (1, lambda x : np.isclose(x[0], x_max), dolfinx.fem.Constant(mesh, 1000.0))
...
set_point = 0.10
F_g = ufl.as_tensor((
    (1+0.1*ufl.sqrt(1 + 2*(elastic_strain[0, 0])) - set_point, 0, 0),
    (0, 1+0.1*ufl.sqrt(1 + 2*(elastic_strain[1, 1])) - set_point, 0),
    (0, 0, 1+0.1*ufl.sqrt(1 + 2*(elastic_strain[2, 2])) - set_point)))
