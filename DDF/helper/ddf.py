...
def eval_expression(expr, mesh, point = [0.5, 0.5, 0.5]):
    ...
    _, _, owning_points, cells  = dolfinx.cpp.geometry.determine_point_ownership(
        mesh._cpp_object, arg1 , 1e-2)  # The last input value is normally 1e-6
...
def neumann_or_robin(marker):
    ...
    def get_first_digit(x): return int(str(abs(x))[0])
...
def natural_injection(mesh, natural_bcs, F, v, u):
    ...
    for marker, _, value in natural_bcs:
        if neumann_or_robin(marker) == "neumann":
            neumann_bcs.append(ufl.inner(v, value * ufl.cofac(F) * n) * ds(subdomain_id=marker))
        elif neumann_or_robin(marker) == "robin":
            robin_bcs.append(value[0]*(ufl.inner(v, (u - value[1] * ufl.cofac(F) * n))) * ds(subdomain_id=marker))
