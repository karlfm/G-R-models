...
def create_box(X: list[list[float]]):
    ...
    ys = X[1]
    zs = X[2]
    return dolfinx.mesh.create_box(MPI.COMM_WORLD, [np.array([xs[0], ys[0], zs[0]]), np.array([xs[-1], ys[-1], zs[-1]])], [len(xs), len(ys), len(zs)], cell_type=dolfinx.mesh.CellType.hexahedron)
...
def get_boundary_nodes(mesh, X):
    ...
    for i, x in enumerate(X):
        ...
        x_min = dolfinx.mesh.locate_entities(mesh, boundary_dimension, lambda x: np.isclose(x[i], x_min_edge))
        x_max = dolfinx.mesh.locate_entities(mesh, boundary_dimension, lambda x: np.isclose(x[i], x_max_edge))
